  ...
  useEffect(() => {
    if (highlightedItem) {
      ...
      const element = document.getElementById(
        `item-${highlightedItem.replace(/\s+/g, "-").toLowerCase()}`
      );
      if (element) {
        setTimeout(() => {
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <motion.p
            ...
            viewport={{ once: true }}
          >
            Transparent pricing for all our services. No hidden fees, just
            quality service.
          </motion.p>
        </motion.div>
          ...
          >
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold">
                    {priceData[activeTab].title}
                  </h3>
                  <p className="text-blue-100 mt-1">
                    {priceData[activeTab].description}
                  </p>
                </div>
                <button
                  ...
                  <tbody>
                    {Object.entries(filteredItems()).map(
                      ([item, details], index) => (
                        ...
                        >
                          <td className="px-4 py-3 text-sm text-gray-800 border-t border-gray-100">
                            {item}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-600 border-t border-gray-100">
                            {details.Type}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-800 font-medium text-right border-t border-gray-100">
                            ₹{details.Price}
                          </td>
                          <td className="px-4 py-3 text-sm border-t border-gray-100 text-right">
                            <button
                              onClick={() =>
                                copyToClipboard(
                                  generateShareUrl(activeTab, item)
                                )
                              }
                    ...
                    {Object.keys(filteredItems()).length === 0 && (
                      <tr>
                        ...
                        >
                          No items found matching your search. Try a different
                          search term.
                        </td>
                      </tr>
                    )}
            ...
            <div className="bg-gray-50 px-6 py-4 border-t border-gray-100">
              <p className="text-sm text-gray-500">
                * Prices may vary based on fabric type, stains, and size.
